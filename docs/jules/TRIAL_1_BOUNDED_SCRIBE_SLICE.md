...
The launch message supplies an exact expected branch and full expected Git SHA. Run `git status --short --branch`, `git rev-parse --abbrev-ref HEAD`, and `git rev-parse HEAD` before editing. If the checkout is dirty, the branch is not `production/core-foundation`, or HEAD differs from the supplied SHA, make no changes and finish with `BASE_MISMATCH`, showing only the observed branch and SHA. Never inspect or print `.env*` files or values. Do not use any legacy checkout, branch, archive, or copied implementation.
