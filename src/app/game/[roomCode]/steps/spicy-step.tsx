...
export function SpicyStep({ gameState, me, handlers }: SpicyStepProps) {
  ...
  const handleSpicySelect = async (value: SpicyLevel['name']) => {
    ...
    if (allReady) {
      const playerLevels = updatedPlayers.map((p) =>
        SPICY_LEVELS.findIndex((l) => l.name === p.selectedSpicyLevel!)
      );
  ...
  return (
    <div className="w-full max-w-5xl text-center">
      <h1 className="text-5xl md:text-6xl font-bold leading-tight">Set the Mood</h1>
      ...
      <div
        className={cn(
          ...
          players.length === 2 ? 'sm:grid-cols-2' : 'sm:grid-cols-3'
        )}
      >
        {players.map((player) => (
